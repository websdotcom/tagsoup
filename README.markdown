...
------------
   ...
   TagSoup is a SAX-compliant parser written in
   Java that, instead of parsing well-formed or valid XML, parses HTML as
   it is found in the wild: [poor][1], nasty and brutish, though quite often
   ...
   licensed under the [Apache License][2], Version 2.0, which allows
   ...
   project, feel free to ask [John Cowan][13].)
...
-----------------------------------------------------------
   ...
   TagSoup will not build out of the box. You need to retrieve [Saxon
   6.5.5][3], which does not have the bug. Unpack the zipfile in an empty
...
* The TagSoup core now supports character entities with values above
  ...
  standard character entities from the [2007-12-14 draft of XML
  Entity Definitions for Characters][4], except the 94 which require more
...
[Download][5] the TagSoup 1.2 jar file here. It's about 87K long.

[Download][7] the current CHANGES file here.
...
-----------------
   ...
   intended to permanently clean up bad HTML, as [HTML Tidy][8] does, only
   ...
   error: the TagSoup motto is ["Just Keep On Truckin'"][9]. But there's
...
`This is <b>bold, <i>bold italic, </i></b><i>italic, </i>normal text.`
   ...
   with any framework that can accept SAX parsers. In particular, [XOM][10]
   is known to work.

   You can replace the low-level HTML scanner with one based on Sean
   McGrath's [PYX format][11] (very close to James Clark's ESIS format). You
   ...
   consider trying to adapt [the Mozilla one][12]; if you succeed, let me
   know.
...
----------------------------------
   ...
   [John Cowan][13] is also distributing [TSaxon][14], a repackaging of
   version 6.5.5 of Michael Kay's Saxon XSLT version 1.0 implementation
   that includes TagSoup. TSaxon is a drop-in replacement for Saxon, and
   can be used to process either HTML or XML documents with XSLT
   stylesheets.
...
----------------
   ...
   [Extreme Markup Languages 2004][15] about TagSoup, updated from the one
   ...
   [OpenDocument][16] [Powerpoint][17] [PDF][18].

   I also had people add ["evil" HTML][19] to a large poster so that I could
   [clean it up][20]; View Source is probably more useful than ordinary
...
<pre>
  ...
  Ecritez une balise ouvrante (sans attributs)<br/> ou fermante HTML ici, s.v.p.<p/>
   ...
   There is a [tagsoup-friends][21] mailing list hosted at [Google Groups][22].
   You can [join via the Web][23], or by sending a blank email to
   [tagsoup-friends+subscribe@googlegroups.com][24]. The [archives][25] are
   open to all.
   ...
   [5]: http://github.com/downloads/websdotcom/tagsoup/tagsoup-1.2.jar
   [6]: http://www.saxproject.org/apidoc/org/xml/sax/package-summary.html
   [7]: http://github.com/websdotcom/tagsoup/raw/master/CHANGES
   [8]: http://tidy.sf.net/
  ...
  [21]: http://groups.google.com/group/tagsoup-friends
  [22]: http://groups.google.com/
  [23]: http://groups.google.com/group/tagsoup-friends/subscribe
  [24]: mailto:tagsoup-friends+subscribe@googlegroups.com
  [25]: http://groups.google.com/group/tagsoup-friends/topics